...
**Where this sits in the stack:** `phionyx-eval-inspect` (v0.1.0) is an **adapter**, not the engine, the gate, or the evidence format. It is a read-only bridge that converts the signed evidence chain produced by the Phionyx runtime into Inspect AI's native log format. The components it sits alongside are the SDK/engine [`phionyx-core`](https://pypi.org/project/phionyx-core/) (current release on PyPI), the self-governance gate [`phionyx-pipeline-mcp`](https://github.com/halvrenofviryel/phionyx-pipeline-mcp) (current release on PyPI), and the vendor-neutral [AI Runtime Evidence Protocol (AIREP)](https://github.com/halvrenofviryel/ai-runtime-evidence-protocol) — an experimental proposed evidence-receipt format; Phionyx emits RGE records, and no released conformant RGE→AIREP projection exists yet. See [Companion packages](#companion-packages--the-wider-stack) and [Evidence format — AIREP](#evidence-format--airep) below.
